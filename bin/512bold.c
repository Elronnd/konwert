...
void update_pal(void) {
	...
	for (int i = 0; i < 8; i++)
		if (zmien[i]) {
			printf("\33]P%c%s", hex[i], normalny[nastare[i]]);
			zmien[i] = 0;
		}
	zmiencos = 0;
}
...
void setterm_command(void) {
	switch(par[0]) {
		...
		case 8: /* store colors as defaults */
			def_color = ((underline ? ulcolor :
						intensity == 0 ? halfcolor :
						(color & 0x0F)) ^ (intensity == 2) * 8)
				| (color & 0xF0);

			printf("%d;%d", 30 + (def_color & 0x07), 40 + (def_color >> 4));
			if (def_color & 0x08) {
				printf(";1");
			}
