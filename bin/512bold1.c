...
int color, intensity, underline, s_color, s_intensity, s_underline, npar, par[NPAR];

void update_attr(void) {
	printf("%d;%d",
			30 + (underline ? ulcolor : intensity == 0 ? halfcolor : kolory[intensity == 2][color & 0x0F]),
			40 + kolory[0][color >> 4]);
}

void default_attr(void) {
	color = def_color;
	intensity = 1;
	underline = 0;
}
...
int main(void) {
	reset_terminal();
	printf("\33]P0000000\33]P1AA0000\33]P200AA00\33]P3FFFFFF"
